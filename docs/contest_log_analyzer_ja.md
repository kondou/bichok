...
iPhone（Safari）でも利用可能。縦・横いずれの向きでも動作する。

### 推奨する使い方

SkookumNet ライブ接続時のリアルタイムモニターとして、**ペインビュー**（→ [§16](#16-ペインビューローリングウィンドウ)）の使用を推奨する。ペインビューはコンパクトな表示に最適化されており、スマートフォンの画面でも視認性が高い。

### 上部を隠す / 表示ボタン

スマートフォンでペインビューを使用している場合、画面左下に **「▲ 隠す」** ボタンが表示される。

![スマートフォン ペインビュー（上部表示状態）](images/L-1.png)

タップするとヘッダー・統計バー・コントロール類が非表示になり、ペインの表示領域が広がる。

![スマートフォン ペインビュー（上部非表示状態）](images/L-2.png)

非表示状態では **「▼ 表示」** ボタンが同じ位置に表示される。再度タップすると元の表示に戻る。
...
> このボタンはペインビュー表示中（SkookumNet ライブ接続時）のみ現れる。縦・横どちらの向きでも動作する。
